...
# st.write("""
# Upload a video file to analyze the facial expressions within it. 
# The application will process the video frame by frame and display the detected emotions.
# """)
...
#     st.markdown("""
#     1.  **Upload a Video:** Use the file uploader in the main panel to select a video file (e.g., MP4, AVI, MOV).
#     2.  **Start Analysis:** Click the **"Analyze Video"** button.
#     3.  **View Results:** The video will play with detected faces highlighted and their corresponding emotions displayed. The analysis for the current frame will appear on the right.
#     """)
#     st.header("Notes")
#     st.markdown("""
#     - Processing time depends on the video length and your computer's performance.
#     - The model works best with clear, well-lit, front-facing faces.
#     - If no face is detected in a frame, it will be noted in the analysis results.
#     """)
...
#             if not cap.isOpened():
#                 st.error("Error: Could not open video file. It might be corrupted or in an unsupported format.")
#             else:
#                 st.success("Video loaded successfully! Starting analysis...")
#                 emotion_text = ""
...
#                             dominant_emotion = face_data['dominant_emotion'].capitalize()
...
#                     # Update the emotion log and progress bar
#                     emotion_log.markdown(f"**Current Frame Emotion:**\n{emotion_text}")
#                     progress_bar.progress(frame_count / total_frames)

#                 # Release the video capture object and clean up the temp file
#                 cap.release()
#         finally:
